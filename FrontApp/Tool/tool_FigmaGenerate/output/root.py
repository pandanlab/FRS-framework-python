...
class Root:
    ...
    def change_index(self, number):
        if number   == 1: 
            import FrontApp.Tool.tool_FigmaGenerate.output.Page.Frame1 as page1
            self.change_Frame(page1)
        elif number == 2: 
            import FrontApp.Tool.tool_FigmaGenerate.output.Page.Frame2 as page2
            self.change_Frame(page2)
        elif number == 3: 
            import FrontApp.Tool.tool_FigmaGenerate.output.Page.Frame3 as page3
            self.change_Frame(page3)
        elif number == 4: 
            import FrontApp.Tool.tool_FigmaGenerate.output.Page.Frame4 as page4
...
def show_frame():
    app_controll = tk.Tk()
    label = tk.Label(app_controll,text=1)
    label.pack()
    button = tk.Button(app_controll,text="move")
    button.pack()
    ...
    def move_page():
        global page
        hello.change_index(page)
        label.configure(text=str(page))
        page += 1
        if(page>number): page = 1

    button.configure(command=move_page)
    app_controll.mainloop()
import threading
